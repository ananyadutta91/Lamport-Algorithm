...
public class Frame extends JFrame {
    ...
    public void addQueued(int node, int num) {
        tickets[node] = num;
        for (int i = 0; i <Screen.node.length+1; i++) {
			int q = queued[i];
            if (q == node) return;
            else if (q == -1) { 
                queued[i] = node;
                break;
                    ...
                    for (int j =Screen.node.length; j > i; j--)
                        queued[j] = queued[j-1];
                    queued[i] = node;
                    break;
             }
    ...
    private void setGraphics() {
        for (int i = 0 ; i <Screen.node.length; i++) {
            int q = queued[i];
            if (q == -1) break;
            lines[i].setLine(
    ...
    public void removeQueued() {
        tickets[queued[0]] = 0;
        for (int i = 1; i <Screen.node.length+1; i++)
            queued[i-1] = queued[i];
        repaint();
    }
    ...
    private class RAPanel extends JPanel {
        public void paintComponent(Graphics g) {
            ...
            for (int i = 0 ; i <Screen.node.length; i++) {
                int q = queued[i];
                if (q == -1) break;
                g2d.setColor(Screen.colors[q]);  // Color of receiving node		
